...
8. IMAGES & HERO TEXT READABILITY — if AI-generated images are provided (img/hero.jpg, img/about.jpg, img/services.jpg), they MUST be used as prominent, full-width or large-area visuals — NOT tiny thumbnails. The hero image MUST fill the entire hero width. The about image must be at least 40% section width on desktop. Add `object-fit: cover` to `<img>` elements placed in fixed-size containers. Service cards must NOT use emojis as the primary icon — use CSS styling instead.
   HERO OVERLAY — CRITICAL: When a <img> tag is used as the hero background (alongside a sibling overlay div), the overlay MUST have a POSITIVE z-index (z-index: 1) so it renders ABOVE the image. NEVER use z-index: -1 for an overlay inside a parent that already has a negative z-index (e.g. .hero-bg { z-index: -2 }) — this creates a stacking context where z-index: -1 places the overlay BEHIND the image, making it invisible and leaving text unreadable. The correct pattern is:
   .hero-bg { position: absolute; top:0; left:0; width:100%; height:100%; z-index: -1; }
   .hero-img { position: absolute; top:0; left:0; width:100%; height:100%; object-fit: cover; z-index: 0; }
   .hero-overlay { position: absolute; top:0; left:0; width:100%; height:100%; z-index: 1; background: linear-gradient(135deg, rgba(0,0,0,0.65) 0%, rgba(0,0,0,0.45) 60%, rgba(0,0,0,0.65) 100%); }
   .hero-content { position: relative; z-index: 2; color: #fff; }
   The overlay gradient opacity MUST be at least 0.55 at its lightest point to guarantee 4.5:1 contrast for white text. Additionally, always add `text-shadow: 0 2px 8px rgba(0,0,0,0.6)` to hero headings as a secondary safety net. Using the brand's primary color in the gradient (e.g. rgba(primary-r, primary-g, primary-b, 0.7)) instead of pure black creates a more branded, intentional look while maintaining readability.
